...
import { BadgeViewer } from "../../config/BadgeViewer";
import { features } from "../../config/features";

export function BadgesView(props: { id: string }) {
  const UIComponents = useUIComponents();
  const router = useRouter();
  const t = useTranslate();
  const collection = use(BadgesCollection);
  ...
  if (isLoading) {
    ...
    if (error || document === null) {
      ...
      content = <BadgesViewComponent document={document} />;
    }
  }
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.badges.view.header")}
...
export function BadgesViewComponent(props: { document: Partial<Badge> }) {
  const document = props.document;

  const viewer = use(BadgeViewer, { transient: true });
...
export function getHeaderActions(
  ...
  if (features.edit) {
    actions.push(
      <Link
        key="edit"
        to={router.path(Routes.BADGES_EDIT, {
          params: { id },
        })}
      >
      ...
      <Ant.Popconfirm
        ...
        onConfirm={() => {
          collection.deleteOne(id).then(() => {
            router.go(Routes.BADGES_LIST);
            Ant.notification.success({
              message: "Success",
              description: "You have deleted the Badge",
            });
          });
        }}
      >
  ...
  const viewRoutePath = router.path(Routes.BADGES_VIEW, {
    params: { id },
  });
