...
import { BadgeList } from "../../config/BadgeList";

export class BadgesAntTableSmart extends AntTableSmart<Badge> {
  collectionClass = BadgesCollection;

  getBody(): QueryBodyType<Badge> {
    return BadgeList.getRequestBody();
  }

  getColumns(): ColumnsType<Badge> {
    const list = this.container.get(BadgeList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<Badge> {
    return {
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.BADGES_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.BADGES_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
