...
import { EndUserListFiltersForm } from "../../config/EndUserListFiltersForm";

type EndUsersListFiltersProps = {
  onUpdate: (filters: any) => void;
};

export const EndUsersListFilters = React.memo(EndUsersListFiltersBase);

export function EndUsersListFiltersBase(props: EndUsersListFiltersProps) {
  ...
  const filterForm = use(EndUserListFiltersForm, { transient: true });
  filterForm.build();
