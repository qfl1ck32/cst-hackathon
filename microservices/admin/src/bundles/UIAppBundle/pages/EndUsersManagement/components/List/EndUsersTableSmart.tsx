...
import { EndUserList } from "../../config/EndUserList";

export class EndUsersAntTableSmart extends AntTableSmart<EndUser> {
  collectionClass = EndUsersCollection;

  getBody(): QueryBodyType<EndUser> {
    return EndUserList.getRequestBody();
  }

  getColumns(): ColumnsType<EndUser> {
    const list = this.container.get(EndUserList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<EndUser> {
    return {
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.END_USERS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.END_USERS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
